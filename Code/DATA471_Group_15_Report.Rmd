...
We are working on the time series dataset. As our groups specific series, we have used the New Zealand Activity Index (NZAC). This is a part of the Covid-19 Dataset and was first used in 2020 but has been extended using historical data to 2004. It is a monthly index that is a summary dataset that pulls on eight constituent datasets. These datasets include consumer spending, job vacancies, electricity generation, and traffic data. The data is sourced from both public and private institutions (e.g. Ministry of Social Development, ANZ).

This is a time-series index, where the 8 component datasets are all numerical time-series. It is constructed using a weighted average of the components, and it aims to show the common movements across the constituent datasets. It is then scaled so that it has the same mean and variance as year-on-year Real Gross Domestic Product (RGDP) growth. This style of activity index became common during the covid-19 pandemic, when decision-makers in government and business were interested in higher frequency data than has previously been on offer. The NZAC is based on similar indices in the United States of America, which were developed by the Chicago Federal Reserve and the New York Federal Reserve. These provide weekly and monthly activity indices for the US and use the same weighted average method that is used by the Treasury for the NZAC.

This dataset is important as it is higher frequency than most economic datasets. Measures of GDP and the unemployment rate are released quarterly and can be revised for up to 2 years following the release. It is well understood in the economics sector that accurate data can carry a 6-to-18-month lag, especially when considering the whole economy. These smaller indexes enable a faster release of data that is more up to date but reduces the noise that can be present in the component data sets. The NZAC is highly correlated with Real GDP, which means that it could be helpful when making economic forecasts, or if you needed to know how the economy was performing before the quarterly GDP updates are released.

The dataset allows you to tell a clear economic story, especially when you look at how it corresponds to its component datasets, and data from different economic sectors e.g., construction or tourism. You can clearly see features such as the 2008 global financial crisis, and the impact of the major covid-19 lockdowns in New Zealand. This could support answering questions about how economic activity has changed after covid, by looking at which components have become more significant, or how the correlation to other datasets has changed post-covid. 

There are no significant periods of missing date in the main index that we used, however some of the other data that we are using to assess the correlation of the NZAC does have some missing data. We did not have to directly combine the NZAC with the other datasets, as they are all time series, so we can merge based on the date-time index. 
